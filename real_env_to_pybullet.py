...
def detect_cue_ball():
    ...
    detect_balls()와 동일한 카메라/ArUco/호모그래피 파이프라인을 쓰지만
    빨강/노랑 목적구는 요구하지 않고, 흰 공만 검출되면 즉시 반환한다.
    마찰 계수 보정처럼 큐볼 한 개만 필요한 경우에 사용한다.

    Returns:
        cue_pos: [x, y, z] (m, PyBullet 좌표계, 위치 오프셋 자동 적용)
    """
    # Calibration Load
    calib = np.load("calibration_result.npz")
    K = calib["K"]
    ...
    # calibration 적용
    newK, roi = cv2.getOptimalNewCameraMatrix(K, dist, (1280, 720), 1, (1280, 720))
    mapx, mapy = cv2.initUndistortRectifyMap(K, dist, None, newK, (1280, 720),
                                             cv2.CV_32FC1)
    ...
    def get_homography_from_aruco(frame):
        ...
        for rid in required_ids:
            if rid not in marker_dict:
                ...
                return None
        src_pts = np.array([
            marker_dict[0], marker_dict[1], marker_dict[2], marker_dict[3]
        ], dtype=np.float32)
        dst_pts = np.array([
            [aruco_size/2, aruco_size/2],
            [DISPLAY_WIDTH - aruco_size/2, aruco_size/2],
    ...
    def pixel_to_table(cx, cy):
        table_x = (cx / DISPLAY_WIDTH) * TABLE_WIDTH_MM
        table_y = ((DISPLAY_HEIGHT - cy) / DISPLAY_HEIGHT) * TABLE_HEIGHT_MM
        return table_x, table_y
    ...
    def detect_ball(mask, frame, color):
        ...
        for cnt in contours:
            ...
            if bbox_area > max_area:
                ...
                table_x, table_y = pixel_to_table(cx, cy)
                best_ball = {
                    "position": (float(table_x), float(table_y)),
                    "center": (cx, cy),
                    "radius": radius
                }
        if best_ball is not None:
    ...
    while True:
        ...
        undistorted = cv2.remap(color_image, mapx, mapy, cv2.INTER_LINEAR)

        H = get_homography_from_aruco(undistorted)
        if H is None:
            print("Need ArUco markers 0,1,2,3")
            continue

        warped_color = cv2.warpPerspective(undistorted, H,
                                           (DISPLAY_WIDTH, DISPLAY_HEIGHT))
        hsv = cv2.cvtColor(warped_color, cv2.COLOR_BGR2HSV)
        white_mask = cv2.inRange(hsv, WHITE_LOWER, WHITE_UPPER)
        white_mask = remove_corner_regions(white_mask)

        white_ball = detect_ball(white_mask, warped_color, (255, 255, 255))
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            pipeline.stop()
            cv2.destroyAllWindows()
            raise RuntimeError("흰 공 검출 취소됨")

    white_ball = [white_ball[0] / 1000.0, white_ball[1] / 1000.0]

    # 좌표 변환: 카메라 → PyBullet 좌표계 (detect_balls와 동일)
    L = MAZE_TABLE_LENGTH
    W = MAZE_TABLE_WIDTH
    H = MAZE_TABLE_SURFACE_HEIGHT
    CX = MAZE_TABLE_CENTER_X
    ...
    center = np.array([CX, CY, H])
    x_offset = 0.3 + L + 2 * thickness
    y_offset = (W + 2 * thickness) / 2 - float(center[1])

    cue_pos = [x_offset - float(white_ball[1]),
               float(white_ball[0]) - y_offset,
               float(ball_h)]

    # 캘리브레이션 오프셋 자동 적용
    pos_offset = load_position_offset()
    cue_pos[0] = float(cue_pos[0] + pos_offset.get('x', 0.0))
    cue_pos[1] = float(cue_pos[1] + pos_offset.get('y', 0.0))

    return cue_pos


def wait_real_cue_ball_stop(interval=0.5, threshold_mm=3.0, max_wait=10.0,
                            verbose=True):
    """흰 공(큐볼) 하나만으로 정지 여부 판단 (마찰 보정용).

    detect_cue_ball()를 interval초 간격으로 호출해 큐볼 변위가
    threshold_mm 미만이면 정지로 판단한다.

    Returns:
        cue_pos: [x, y, z] — 최종 정지 위치
    """
    ...
    while _time.time() - start < max_wait:
        ...
        if prev is not None:
            disp = np.linalg.norm(
                np.array(current[:2]) - np.array(prev[:2])) * 1000  # m → mm
            if verbose:
                print(f"  [STOP] 큐볼 변위: {disp:.1f}mm")
            if disp < threshold_mm:
                if verbose:
                    print(f"  [STOP] 큐볼 정지 확인 ({_time.time()-start:.1f}초)")
                return current
    ...
    return prev if prev is not None else detect_cue_ball()

...
1. **로봇 팔-공 물리 간섭**: 타격 후 로봇 팔이 Home으로 복귀하는 과정에서 공이 로봇 링크에 부딪혀 궤적이 왜곡됨. 현재 충돌 필터가 비활성화 되어 있어 물리적으로는 관통하지만, PD 제어기의 관절 위치 이동이 공을 밀어내는 현상 발생
2. **2D-3D 물리 gap**: 2D 계획기가 PyBullet 변수(반발계수, 마찰)를 동기화하지만, 3D 효과(스핀, 수직 바운스, 접촉 역학)는 근사만 가능. 계획기에서 hit_t1=True, hit_t2=True인 경로가 실제 PyBullet에서는 0.08~0.15m 빗나감
3. **장애물 클리어런스 딜레마**: 클리어런스를 크게(12cm) 잡으면 모든 후보가 거부되고, 작게(5cm) 잡으면 로봇 팔이 장애물과 겹침. 현재 7cm로 타협 중이나 장애물 배치에 따라 전부 거부될 수 있음
4. **로봇-환경 충돌 비활성화**: 로봇 링크와 환경 객체(장애물, 벽, 바닥) 간 물리 충돌이 전면 비활성화. EE 위치 기반 클리어런스 체크만 수행하므로 팔 중간 링크가 장애물을 관통할 수 있음
5. **비전 시스템 미구현**: SCAN에서 PyBullet API로 위치 직접 읽음
6. **타격 속도 일관성**: PD 제어기 특성상 관절 구성에 따라 실제 EE 속도가 계획 대비 변동

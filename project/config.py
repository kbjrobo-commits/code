...
MAX_TOOL_SPEED = 0.8           # 최대 툴 속도 (m/s) — 로봇 물리적 한계
...
# 타격 파라미터 (공통)
# ============================================================
STRIKE_APPROACH_DIST = 0.1    # 타격 전 접근 거리 (m) — 시뮬/실제 균형 (0.08→0.10)
STRIKE_FOLLOW_DIST = 0.05    # Follow-through 거리 (m) — 테이블 관통 방지용 축소
...
TOOL_HORIZONTAL_EXT = 0.015      # 꺾인 후 수평 연장 (m)
...
TOOL_YAW_OFFSET = 0  # 30° 오프셋 (rad) — 타격 방향과 EE x축이 30° 어긋남
...
MAZE_TABLE_SURFACE_HEIGHT = 0.115  # 테이블 바닥면 높이 (m)
MAZE_TABLE_CENTER_X = 0.535        # 테이블 중심 X (원래 위치 복원)
...
MAZE_CUSHION_RESTITUTION = 0.7   # 쿠션 반발계수
MAZE_BALL_RADIUS = 0.0125           # 큐볼 반지름 (m) — 지름 24mm
MAZE_BALL_MASS = 0.014              # 큐볼 질량 (kg) — 가벼운 공
MAZE_BALL_RESTITUTION = 0.8       # 큐볼 반발계수
MAZE_BALL_FRICTION = 0.26        # 실제 당구공 수준 (0.3은 과도)
MAZE_BALL_ROLLING_FRICTION = 0.05  # 올림: 실측 기반 보정 (0.008→0.012)a
...
# 포켓볼 데모 파라미터
# ============================================================
POCKET_DEMO_CUSHION_RESTITUTION = 0.7  # 쿠션 반발계수 (낮음, 붙는 현상)
...
LINEUP_SPACING = 0.09                # 목적구 간 간격 (m)
LINEUP_CUE_OFFSET = 0.12             # 큐볼-첫 목적구 간격 (m)

# ============================================================
# Escape shot parameters
# ============================================================
ESCAPE_WALL_GAP_THRESHOLD = 0.02 + MAZE_BALL_RADIUS
ESCAPE_STRIKE_HEIGHT_OFFSET = 0.0185
ESCAPE_BALL_SPEED = 0.3
ESCAPE_SAFE_APPROACH_DIST = 0.03
